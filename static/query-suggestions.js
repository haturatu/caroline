...
let visibleQuerySuggestions = [];
let querySuggestionIndex = 0;
function queryToken(input) {
...
function matchingSuggestions(items, token) {
    const normalized = token.toLowerCase();
    return items.filter((item) => item.label.toLowerCase().startsWith(normalized));
}
function valueSuggestions(field, token, start, end) {
    let values = [];
    switch (field) {
        case "severity":
    ...
    return matchingSuggestions([...new Set(values)].map((value) => ({
        label: value,
        detail: `${field} value`,
        replacement: `"${value.replace(/"/g, '\\"')}"`,
        replaceStart: start,
        replaceEnd: end,
    })), token.replace(/^"/, "").replace(/"$/, "")).slice(0, 8);
}
function operatorSuggestions(field, start, end) {
...
function buildQuerySuggestions(input) {
    const { line, lineStart, token, tokenStart } = queryToken(input);
    const before = line.trimStart();
    const valueMatch = before.match(/^([A-Za-z0-9_.-]+)\s*(>=|<=|!=|=|:|>|<)\s*([A-Za-z0-9_.-]*)$/);
    if (valueMatch) {
        const valueStart = lineStart + line.lastIndexOf(valueMatch[3]);
        return valueSuggestions(valueMatch[1], valueMatch[3], valueStart, input.selectionStart);
    }
...
export function closeQuerySuggestions() {
...
export function renderQuerySuggestions() {
    ...
    querySuggestionIndex = Math.min(querySuggestionIndex, visibleQuerySuggestions.length - 1);
    suggestions.innerHTML = visibleQuerySuggestions
        .map((suggestion, index) => `<button class="query-suggestion${index === querySuggestionIndex ? " active" : ""}" type="button" role="option" aria-selected="${index === querySuggestionIndex}" data-suggestion-index="${index}" id="query-suggestion-${index}"><span class="query-suggestion-label">${escapeHTML(suggestion.label)}</span><span class="query-suggestion-detail">${escapeHTML(suggestion.detail)}</span></button>`)
        .join("");
    suggestions.removeAttribute("hidden");
    input.setAttribute("aria-expanded", "true");
    input.setAttribute("aria-activedescendant", `query-suggestion-${querySuggestionIndex}`);
    $$(".query-suggestion").forEach((button) => button.addEventListener("mousedown", (event) => event.preventDefault()));
    $$(".query-suggestion").forEach((button) => button.addEventListener("click", () => applyQuerySuggestion(Number(button.dataset.suggestionIndex))));
}
function applyQuerySuggestion(index) {
    ...
    const cursor = suggestion.replaceStart +
...
export function handleQueryKeydown(event) {
