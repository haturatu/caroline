...
import { closeQuerySuggestions, handleQueryKeydown, renderQuerySuggestions, } from "./query-suggestions.js";
import { renderAll, renderDetail, renderEntries, renderFilters, renderLoading, renderResultsMeta, setRenderActions, } from "./render.js";
import { state } from "./state.js";
let refreshTimer = null;
let detailReturnFocusId = null;
...
function showError(message, source = "explorer") {
...
function applyTheme(theme) {
    ...
    if (themeColor)
        themeColor.content = theme === "dark" ? "#202124" : "#f8fafd";
...
function focusSection(section) {
    ...
    setActiveNavigation(section === "main-content"
        ? "logsNavButton"
        : section === "timeline"
            ? "timelineNavButton"
            : "fieldsNavButton");
...
async function loadStatus() {
    try {
        ...
        if (status.connected) {
            ...
            showError("Docker is unavailable. Start Docker and mount /var/run/docker.sock into Caroline.", "status");
        }
    }
    catch (error) {
...
async function loadExplorer(append = false) {
    ...
    try {
        ...
        if (append) {
            ...
            state.entries = [
                ...
                ...incoming.filter((entry) => {
                    if (existingIds.has(entry.insertId))
                        return false;
        ...
        if (response.errors?.length)
            showError(`Some containers could not be read: ${response.errors.join(" · ")}`, "explorer");
    }
    catch (error) {
        if (!append && !state.response) {
            state.entries = [];
...
function runQuery() {
    ...
    state.searchText = $("#searchAllFields").value.trim();
    state.pageToken = "";
    closeQuerySuggestions();
    syncURL();
    void loadExplorer();
}
function shiftTimelineRange(step) {
    ...
    state.timeTo = "";
    state.pageToken = "";
    $("#rangeFilter").value = state.duration;
    syncURL();
    void loadExplorer();
}
...
function applyCustomRange() {
    const from = fromDateTimeLocal($("#customFromInput").value);
    const to = fromDateTimeLocal($("#customToInput").value);
...
function setupEvents() {
    $("#consoleMenuButton").addEventListener("click", () => {
        ...
        $("#consoleMenuButton").setAttribute("aria-expanded", String(state.navExpanded));
    });
    $("#globalSearchButton").addEventListener("click", focusGlobalSearch);
    $("#headerMenuButton").addEventListener("click", () => toggleMenu($("#headerMenu"), $("#headerMenuButton")));
    ...
    $("#logsNavButton").addEventListener("click", () => focusSection("main-content"));
    $("#timelineNavButton").addEventListener("click", () => focusSection("timeline"));
    ...
    $("#searchAllFields").addEventListener("input", (event) => {
    ...
    $("#queryInput").addEventListener("input", () => {
        state.draftQuery = $("#queryInput").value;
    ...
    document.addEventListener("click", (event) => {
        ...
        if (!target.closest(".query-editor"))
            closeQuerySuggestions();
        if (!target.closest(".global-header"))
            closeHeaderMenus();
    });
    $("#containerFilter").addEventListener("change", (event) => {
        state.container = event.target.value;
        state.pageToken = "";
        syncURL();
        void loadExplorer();
    });
    $("#streamFilter").addEventListener("change", (event) => {
    ...
    $("#shareButton").addEventListener("click", () => {
        ...
        if (copy)
            void copy.then(() => toast("Query link copied."));
        else
            toast("Copy the current URL from the address bar.");
    });
    $("#queryHelpButton").addEventListener("click", () => toast('Use field = value, field >= value, SEARCH("text"), AND, and OR.'));
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (event.key === "Escape") {
            if (!$("#detailDrawer").hasAttribute("hidden"))
                closeDetail();
            else
                closeHeaderMenus();
        }
        if (event.key === "Tab" && !$("#detailDrawer").hasAttribute("hidden")) {
            const focusable = $$("#detailDrawer button, #detailDrawer [href], #detailDrawer input, #detailDrawer textarea").filter((element) => !element.hasAttribute("disabled"));
            if (!focusable.length)
                return;
